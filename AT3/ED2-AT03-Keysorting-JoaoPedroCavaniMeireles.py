...
def lerCabecalho(input1):
    ...
    return size, top, qtde, sort, order, cabecalho
...
def verificacao(entrada, saida):
    ...
    if o <= 2:
        final = open(saida, "w")
        final.writelines("Arquivo Invalido")
        print("-----------------ERRO NO ARQUIVO,ENCERRANDO PROGRAMA--------------------")

        exit()


######## Imprime arquivo de saida ##########
def imprimeArq(vetorimpressao, cabecalho, saida):
    final = open(saida, "w")
    o = 0
    final.writelines(cabecalho)
    final = open(saida, "a")

    while o < len(vetorimpressao):
        string = str(vetorimpressao[o])
        vetorimpressao[o] = str(string.replace(", ", "|"))
        final.writelines(vetorimpressao[o]+"\n")
        o = o+1
...
def main(entrada, saida):
    ...
    size, top, qtde, sort, order, cabecalho = lerCabecalho(entrada)
    ...
    if order == "D":
        ...
        vetorimpressao = vetoralinhado

    vetorimpressao = vetoralinhado
    imprimeArq(vetorimpressao, cabecalho, saida)
